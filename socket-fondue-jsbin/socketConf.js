/*
- emit to browser, emit to bin, emit fondueDTO
- basically it takes the result of fondue instrumented node/fn calls and emits them to the "jsbin" / emits them to the browser, which then uses that info to render nodes on the isopleth screen
*/
...
module.exports = function (httpServer) {
  ...
  var emitToBrowser = function (binId, eventStr, data) { // emit event from socket-fondue-jsbin to browser
    if (binId) {
  ...
  io.on("connection", function (socket) {
    ...
    socket.on("jsbin:listen", function (data) { // jsbin - listen for node changes, understood by socket fondue server 
